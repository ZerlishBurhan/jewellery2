      ...
      {carts.length === 0 ? (
        ...
        carts.map((cart) => (
          <div key={cart.id} style={{ border: "1px solid gray", margin: 10, padding: 10 }}>
            ...
            {/* REFACTOR FIX: Use 'items' instead of 'productIds' */}
            {(!cart.items || cart.items.length === 0) ? (
              ...
              <ul>
                {cart.items.map((item, index) => (
                  <li key={index}>
                    {/* Access fields directly from the item object */}
                    Product: {item.productId} | Quantity: {item.quantity} | Price: {item.finalPrice}
                  </li>
                ))}
              </ul>
            )}
